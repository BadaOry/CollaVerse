...
public class ProductFileProcess {
	public static String productsave(MultipartFile[] prodImg, String location) {
		
		String renamedFileName = null;
		String originalFileName = prodImg[0].getOriginalFilename();
		
		log.info("[FileProcessor] 넘어온 prodImg의 OriginalFileName : {}", originalFileName);
		log.info("[FileProcessor] 넘어온 prodImg의 Upfile location : {}", location);
...
//			log.info("있긴 있는?");
		...
		try {
			...
			prodImg[0].transferTo(new File(location + "/" + renamedFileName));
	...
	public static String productsave1(MultipartFile[] prodImg, String location) {
...
//			log.info("있긴 있는?");
		...
		try {
			// ▼ 업로드한 파일 데이터를 명명 규칙으로 지정한 파일에 저장
			prodImg[1].transferTo(new File(location + "/" + renamedFileName));
	...
	public static String productsave2(MultipartFile[] prodImg, String location) {
...
//			log.info("있긴 있는?");
		...
		try {
			// ▼ 업로드한 파일 데이터를 명명 규칙으로 지정한 파일에 저장
			prodImg[2].transferTo(new File(location + "/" + renamedFileName));
